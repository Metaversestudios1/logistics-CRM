...
const ShowEmployee = () => {
  ...
  const fetchEmployeeDetails = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/getesingleemployee`, {
        method: "POST",
  ...
  const fetchRelatedDetails = async (role, department, teamLead, projects) => {
    try {
      if (role) fetchDataAndUpdateState(role, "role", "http://localhost:3000/api/getSingleRole", "role");
      if (department) fetchDataAndUpdateState(department, "department", "http://localhost:3000/api/getSingleDepartment", "department_name");
      if (teamLead) fetchDataAndUpdateState(teamLead, "team_lead", "http://localhost:3000/api/getesingleemployee", "name");

      // Fetch project names in parallel
      if (projects.length > 0) {
        const projectNames = await Promise.all(projects.map((id) => fetchDataAndGetValue(id, "http://localhost:3000/api/getSingleproject", "name")));
